...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (this.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
            ...
            //获取界面元素
            input = (EditText) findViewById(R.id.input_land);
            mem = (TextView) findViewById(R.id.mem_land);
            tip = (TextView) findViewById(R.id.tip_land);
            _drg = (TextView) findViewById(R.id._drg_land);
            btn[0] = (Button) findViewById(R.id.zero_land);
            btn[1] = (Button) findViewById(R.id.one_land);
            btn[2] = (Button) findViewById(R.id.two_land);
            btn[3] = (Button) findViewById(R.id.three_land);
            btn[4] = (Button) findViewById(R.id.four_land);
            btn[5] = (Button) findViewById(R.id.five_land);
            btn[6] = (Button) findViewById(R.id.six_land);
            btn[7] = (Button) findViewById(R.id.seven_land);
            btn[8] = (Button) findViewById(R.id.eight_land);
            btn[9] = (Button) findViewById(R.id.nine_land);
            div = (Button) findViewById(R.id.divide_land);
            mul = (Button) findViewById(R.id.mul_land);
            sub = (Button) findViewById(R.id.sub_land);
            add = (Button) findViewById(R.id.add_land);
            equal = (Button) findViewById(R.id.equal_land);
            sin = (Button) findViewById(R.id.sin_land);
            cos = (Button) findViewById(R.id.cos_land);
            tan = (Button) findViewById(R.id.tan_land);
            sqrt = (Button) findViewById(R.id.sqrt_land);
            square = (Button) findViewById(R.id.square_land);
            bksp = (Button) findViewById(R.id.bksp_land);
            left = (Button) findViewById(R.id.left_land);
            right = (Button) findViewById(R.id.right_land);
            dot = (Button) findViewById(R.id.dot_land);
            exit = (Button) findViewById(R.id.exit_land);
            drg = (Button) findViewById(R.id.drg_land);
            mc = (Button) findViewById(R.id.mc_land);
            c = (Button) findViewById(R.id.c_land);
            //为数字按键绑定监听器
            for (int i = 0; i < 10; ++i) {
    ...
    private View.OnClickListener actionPerformed = new View.OnClickListener() {
        public void onClick(View v) {
            ...
            //检测输入是否合法
            if(equals_flag == false && "0123456789.()sincostan+-×÷√^".indexOf(command) != -1) {
                //检测显示器上的字符串是否合法
                if(right(str)) {
                    if("+-×÷√^)".indexOf(command) != -1) {
                        //如果输出结果后还存在运算符 则将该运算符压入命令缓存栈中
                    ...
                    //显示器上的字符串不合法时
                    input.setText("0");//将显示器内容变为0
                    vbegin = true;//输入控制为重新输入
                    tip_i = 0;//清空命令缓存栈
                    tip_lock = true;//解锁键盘
                    tip.setText("欢迎使用！");//更改提示的内容
                }
                //将目前的检测内容规定为输入“=”之前的内容
                equals_flag = true;
            }
            ...
            if("0123456789.()sincostan+-×÷√^".indexOf(command) != -1 && tip_lock) {
            ...
            //若输入正确，则将输入信息显示到显示器上
            if("0123456789.()sincostan+-×÷√^".indexOf(command) != -1
                    && tip_lock) { //共25个按键
                print(command);
     ...
     * 只包含0123456789.()sincostan+-×÷√^的是合法的str，返回true
     * 包含了除0123456789.()sincostan+-×÷√^以外的字符的str为非法的，返回false
     */
    ...
    public class calc {
         ...
         * +-基本优先级为1，×÷基本优先级为2， sin cos tan 基本优先级为3，√^基本优先级为4
